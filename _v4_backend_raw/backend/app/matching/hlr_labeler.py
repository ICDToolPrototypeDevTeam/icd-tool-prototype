...
from app.models import HLRLabel, HLRLabelOutput, HLRRequirement
...
def _call_label_api(
    api_key: str, base_url: str, model: str, hlr: HLRRequirement, max_retries: int = 2
) -> HLRLabel:
    """Call DeepSeek API to label one HLR requirement."""
...
def label_hlrs(
    ...
    """Label all HLR requirements via AI, with file-based caching.

    Args:
        hlr_reqs: Parsed HLR requirements.
        api_key: DeepSeek API key (defaults to config).
        base_url: DeepSeek base URL (defaults to config).
    ...
    if not api_key:
        print("  [label] No API key — using fallback (empty labels)", file=sys.stderr)
        return {hlr.requirement_id: _fallback_label(hlr) for hlr in hlr_reqs}

    labels: dict[str, HLRLabel] = {}
    total = len(hlr_reqs)

    print(f"  [label] Labeling {total} HLRs via {model}...")
    for idx, hlr in enumerate(hlr_reqs):
        lbl = _call_label_api(api_key, base_url, model, hlr)
