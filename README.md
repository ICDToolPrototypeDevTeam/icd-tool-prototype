...
本工具用于根据用户输入的EoICD源文件生成条目化需求，并将生成的EoICD条目化需求与软件高层需求进行比对，识别二者之间的不一致问题，最终输出条目化需求文档和差异分析报告。
...
1. 用户上传 EoICD 源文件和软件高层需求文件；
2. 后端解析输入文件，并构建统一分析输入包；
3. CrewAI 多智能体生成多个 EoICD 条目化需求候选结果；
4. 评分智能体和 Python 硬规则评分器对候选结果进行综合评分；
5. 系统选择最佳 EoICD 条目化需求；
