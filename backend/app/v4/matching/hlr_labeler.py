...
- devices: 提取设备/组件/子系统，如 "{device_examples}"。设备名称保持原文形式，中文原文保留中文，英文保留英文。
- signal_keywords: 提取信号/参数/数据项关键词语，中英文都可以，如 "{signal_examples}"。
...
def _build_system_prompt(profile: "ControllerProfile | None") -> str:
    if profile is None:
        device_examples = ", ".join(DEFAULT_DEVICE_EXAMPLES)
        signal_examples = ", ".join(DEFAULT_SIGNAL_EXAMPLES)
    else:
        profile_devices = profile.ai_labeling.device_examples
        profile_signals = profile.ai_labeling.signal_examples
        device_examples = ", ".join(profile_devices) if profile_devices else ", ".join(DEFAULT_DEVICE_EXAMPLES)
        signal_examples = ", ".join(profile_signals) if profile_signals else ", ".join(DEFAULT_SIGNAL_EXAMPLES)
...
def label_hlrs(
    ...
    # Build profile-aware system prompt once for the batch.
