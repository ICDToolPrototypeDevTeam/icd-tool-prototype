...
- **服务器交付包构建脚本与一键部署脚本**：新增 `scripts/build-image.sh`（构建方在本机执行：`npm run build` 生成前端产物 → 构建后端镜像 → `docker save` 导出归档并计算 sha256 → 组装交付目录并写 `BUILD_INFO`）与 `scripts/deploy.sh`（随交付包发出，服务器上一条命令完成「校验归档 → 复核 CPU 架构 → 导入镜像 → 启动 → 健康自检」）。交付包内新增 `BUILD_INFO` 记录镜像标签、架构、归档 sha256 与源码版本，`deploy.sh` 据此在架构不符（避免 `exec format error`）或文件损坏时立刻中止；本机 Compose 不认 `pull_policy` 时自动去掉该行重试。`部署说明.md` 相应重写：以一键部署为主路径，手工步骤等同于脚本行为，并补充「前端产物覆盖式更新无需换镜像」「改动类型与是否需换镜像」等对照表。仓库新增 `.gitattributes`（`*.sh eol=lf`），避免脚本以 CRLF 交付到 Linux 后报 `\r` 类错误。

### Fixed

- 修复第三方 Office（WPS 等）重存过的 .docx / .xlsx 在 Linux 上必然解析失败的问题。这类包的 zip 条目名可能是反斜杠（`word\document.xml`），而 CPython 的 `zipfile` 只在 Windows 上把它换成正斜杠 —— 于是同一份文件本机解析正常、容器与服务器报「文件无法解析」。解析入口现在会先按中央目录原始字节检查条目名，不规范时改写成一份规范副本（与原文件同名，落在原目录的 `_normalized/` 下）再解析，并打一行 `[fix]` 日志；规范文件零额外开销、行为逐字不变。此前各服务器「有的全失败、有的只失败个别文件」的差异由此解释：差别不在系统或镜像，而在文件是否经 WPS 重存过。详见 `docs/development/debug-log.md` BUG-20260923-001。
- 修复上传页「文件预览」对同一类文件显示「无法解析 Word 文件」的问题。预览是**浏览器端**用 mammoth / JSZip 直接解析所选文件，而这套库同样不做 zip 条目名的反斜杠归一化 —— 与上一条同根因，但**任何平台的浏览器都会失败**，且因为预览不经过后端，重建后端镜像对它无效。预览现在会先做一次字节级归一化（等长替换、不重新压缩，规范文件零开销直通），Word 与 Excel 两个分支各过一次；后者实测 SheetJS 本可容忍这类包，属预防性处理。该改动需**重建前端镜像**才生效。详见 `docs/development/debug-log.md` BUG-20260923-002。
- 修复选择「自动识别」系统类型时提交即报 `Internal Server Error`、**任务根本没创建**（因而无任务日志可看）的问题。系统类型自动识别在接口内同步读一次 HLR 文件，不属于后台管线，因而没享受到上一条的归一化、也没有兜底 —— 同一份 WPS 重存的文件在这里抛 `KeyError` 直接变成裸 500。归一化现在覆盖接口侧与追溯表侧（`traceability/trace_parser.py` 5 处、`forward_scope.py` 1 处）的 7 个「直接开上传文件」调用点，判定逻辑与解析结果格式不变。上一轮若手动指定了系统类型则可绕过（该分支不读文件）；该改动需**重建后端镜像**才生效。详见 `docs/development/debug-log.md` BUG-20260923-003。
- 修复上传请求失败后留下「幽灵任务」的问题。此前上传接口先创建任务、再保存上传文件与识别系统类型，后两步失败（文件过大 413、追溯表格式错 422、识别不出系统类型 500）时请求结束、管线线程从未启动，而记录已进任务表：没有线程所以「终止」无效，状态是 `pending` 所以不满足「放弃」的门槛，又没有 `job.json` 所以重启扫描也扫不到——用户只能看着列表里那条「等待开始」。现在改为「登记即承诺运行」：接口用 `JobManager.new_job()` 构造但不登记，`launch_*_pipeline` 在起线程前 `register()`；`abandon` 的门槛改为「没有任何线程在跑」（放行从未启动过的 `pending`，已进入启动窗口的 `pending` 仍须先终止），前端对这类记录也给出「放弃」。详见 `docs/development/debug-log.md` BUG-20260923-004。
- 界面图标由 emoji 改为内联 SVG（`lucide-react`，无新增依赖）。容器镜像里只有 9 种西文字体，emoji 在服务器上会渲染成方框——上传区、预览区、任务状态、结果下载卡与工具入口页共 8 个组件、64 处。保留 `→`、`★/☆`、`▾/▸` 这些由字体覆盖的符号，其余全部换成同名 SVG（如 📄→`FileText`、📊→`FileSpreadsheet`、📋→`ClipboardList`、📎→`Paperclip`、👁️→`Eye`、⏳→`Hourglass`、⚠️→`TriangleAlert`、⏹→`CircleStop`）。**前端改动用覆盖 `frontend/dist` 的方式更新即可，无需重建镜像。**
- 修复**点「终止」后要等第一步跑完才生效**的问题。取消是协作式的（`cancel` 只置标志，须由管线在检查点读取），而检查点原先只埋在**每个 Step 的开头**，Step 1 内部一个都没有 —— 第一步恰恰是「一次调用几十秒到几分钟」的长任务（十万行级 EoICD 表的解析，服务器上更慢），于是终止请求只能等两个大表全部解析完（反向管线还要再等条目化清单生成完）走到 Step 2 开头的检查点才被消费。现在 EoICD 解析器在**整表物化**与**逐行解析**两个循环里按行间隔（5000 行）检查、每张表开头再兜一次（多张小表时行间隔永远数不到）；两个管线的 Step 1 子步骤边界（解析发布方 → 解析订阅方 → 解析 HLR →〔反向〕生成条目化清单）也各补一个检查点。终止由「等整步跑完」变为**秒级响应**；未绑定任务（CLI / 单测）时检查静默 no-op，解析结果与判定逻辑逐字不变。**注意：「第一步慢」本身未变** —— 解析已是 openpyxl 的 `read_only + values_only` 快路径，不引入新依赖没有数量级优化空间，本次只让它随时可停。该改动需**重建后端镜像**才生效。详见 `docs/development/debug-log.md` BUG-20260923-006。
- 修复**大输入时内存峰值把服务器撑爆**的问题（服务器上表现为整机假死，或进程被内核 OOM 击杀）。反向与正向管线各有一处把 87.9MB 的 `eoicd_requirements.json` 整份物化在堆上：Step 1 落盘用 `model_dump_json` + `write_text`（122,674 条时先在堆上造 171MB 的 str，再编出 88MB 的 bytes），收尾又为了取两个整数把它整份反读（`derive_eoicd_hlr_counts`）。现在 Step 1 改为按 2000 行**分块流式写盘**（落盘那一步的内存增量由 +252MB 降到 +0MB，Step 1 峰值 621MB→391MB，且峰值不再随条数放大）；收尾那两个计数改由 `PipelineResult` 随管线回传（新增 `eoicd_count` / `hlr_count`，均带默认值、向后兼容），两个管线与两处 runner 收尾都不再反读。`derive_eoicd_hlr_counts` 保留给旧任务兜底，并把正向结果接口里对它的**无条件调用**改为惰性——原先每打开一次正向结果页就会整份读一遍那 87.9MB。反向管线全局峰值实测 630MB→519MB，落盘文件与原实现**逐字节一致**（对下游 `json.loads` 完全透明）。**注意：改后峰值仍高于服务器可用内存（519MB vs 476MB），升内存仍是主解**，本改动是必要的风险削减——在 RPDU 225,825 条的输入上尤其关键。该改动需**重建后端镜像**才生效。详见 `docs/development/debug-log.md` BUG-20260923-008。
- 修复**正向任务停在 Step 8、看起来「卡死」**的问题。python-docx 的 `row.cells` 并不是「取本行单元格」，它每次调用都用 `iter_tcs()` 把整张表的单元格网格重建一遍（含 vMerge / gridSpan 回溯），而逐行填表时每行要取 6 次 —— 整表因此退化为 O(行数²)：1618 行的正向报告在服务器上十余分钟不产出文件，看着像死锁，实际是平方级重算。现在改为直接读本行 `tr.tc_lst`（`docx_cells.row_cells`，逐行开销只与列数有关；遇横向合并则回落 python-docx 原路径），三个 Word 生成器的逐行填表共 11 处改用它。同名同数据的 800 行对照实测 **19.57s → 1.13s**（17.3×），且 `word/document.xml` 与旧实现**逐字节相同**（同 sha1），1618 行全量 2.73s。顺带给四个报告生成器（含 Excel）补齐协作式取消检查点（Word 每 200 行、Excel 每 5000 行）并打印行数 —— Step 8 现在既能秒级响应终止，也能从日志看出停在哪个报告、多少行。该改动需**重建后端镜像**才生效。详见 `docs/development/debug-log.md` BUG-20260923-009。
- 修复**大输入会把服务器整机拖进假死**的问题（部署层加固）：SSH 与页面全部无响应，连页面上的「终止」都点不动，只能从容器外面杀进程。根因是交付编排文件没给容器设内存上限——容器与宿主共享内存，超限时要么由宿主内核挑最大的进程杀掉（曾击杀 uvicorn），要么整机陷入 swap 抖动。现在后端容器设了内存上限（`mem_limit` 与 `memswap_limit` 取同值，即**该容器不许用 swap**：宁可干脆失败，也不要「慢慢假死」），取值由 `deploy.sh` 从宿主 `/proc/meminfo` 算出（总内存 − 10%，预留不低于 300 MB；上限下限 512 MB；读不到时兜底 640 MB）写入部署目录的根 `.env` 供 Compose 变量替换，启动后还会回读 `HostConfig.Memory` 确认真的落到 cgroup 上（Compose 版本偏低可能不认 `mem_limit`）。设上限后超限**只在容器内** OOM：宿主机始终可用，容器由 `restart` 策略自动拉起，那个任务变成「已中断」，用户在页面上点「放弃」即可。**这是部署层兜底，不替代升内存**：896 MB 的机器上 RPDU 这类 22.5 万条输入依然跑不完，只是不再拖死整机。`部署说明.md` 相应更新（§2 内存要求、§4 目录结构与两个 `.env` 的区别、§5.1 五件事、§6 手工部署步骤、§9 常见问题）。

## [Unreleased] - 2026-09-22

### Added

- **前端全局 MOCK 开关**：顶栏新增开关，开启时全站常驻警示条；提交任务时显式传递该开关，可在容器已配置 MOCK 的环境下双向切换（日志中来源标记为「前端参数」）。
- **任务日志面板**：执行过程中可实时查看后端日志（每 3 秒增量拉取）；进程重启后仍可查看重启前的日志（来自 `job.log` 尾部，最多 256KB / 2000 行）。
- **任务终止**：可在前端终止运行中的任务，任务在当前步骤或 Case 边界停止，已产出的文件保留。
- **失败信息按根因分类**：输入文件 / 文件格式 / 服务配置 / 网络 / 超时 / 限流 / 模型输出 / 全部不可用 / 磁盘 / 已取消 / 内部错误，并提供可执行建议与「一键复制诊断信息」。

### Changed

- **被终止的任务现在可以续跑 / 放弃**：`POST /jobs/{id}/resume` 与 `POST /jobs/{id}/abandon` 的状态门槛由 `interrupted` 放宽到 `interrupted` / `canceled`，前端「未完成的任务」列表把 `canceled` 一并展示并提供「继续 / 放弃」；被终止任务的错误页（「任务已终止」）底部同样给出「继续执行 / 放弃」（复用同一对接口，不再出现「重新尝试」——任务没有失败，文件和已完成判定都还在）。错误视图的收尾动作统一到一行：诊断卡片与日志面板之下是「继续执行 / 放弃」（或失败状态的「重新尝试」）＋弱化的「复制诊断信息」，后者不再埋在诊断卡片内部。续跑按原参数快照重跑、已完成的 LLM 判定照旧复用；续跑前复位取消标志（`Job.clear_cancel()`），否则同一进程内续跑会被上一次的取消事件在第一个检查点立刻再取消。容器重启后已终止的任务仍不载入（`canceled` 不在启动扫描范围），即**续跑只在本次进程存活期间可用**——与 `completed` / `failed` 同待遇。

### Fixed

- 修复前端把「后端任务失败」与「前端轮询超时」当作同一种错误展示、导致无法判断根因的问题；两者现在有明确不同的文案，任务失败原因改从 `GET /api/v4/jobs/{job_id}` 的 `error` 字段读取。
- MOCK 模式下 HLR 标注不再逐条等待 0.2s（16 条 HLR 共 15 次节流等待、末条不等待，实测 3.1s → 0.0s）。
...
- **HLR AI 标注接入中断恢复缓存（Step 2 按条复用）**：中断发生在 Step 2（HLR 标注）时不再整批丢失——每条标注按内容寻址写入 `llm_cache.jsonl`（`kind=hlr_label`，key 含 profile 的 system prompt、需求正文与 requirement_id、调用参数），恢复运行只对未完成的 HLR 真正调用模型；整批完成后仍落盘 `hlr_labels.json` 并直接整体加载（该路径加载的 N 条同样按 `reused` 计入）。失败（重试耗尽）结果用空标签兜底但**不入缓存**，避免把一次失败固化为「已完成」。复用计数口径相应延伸：恢复运行的 `reuse` 现按 Step 2 + Step 4/5/5.5/5.6 累计。范围限反向（正确性）管线，正向（完整性）管线的标注行为与 `reuse` 恒为 0 的契约不变。详见 `docs/architecture/api.md` 第 5 节与第 13.2 节。
