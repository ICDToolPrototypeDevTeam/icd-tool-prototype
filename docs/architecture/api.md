...
| `controller_profile` | str (form) | 否（默认 `ams`） | 控制器 profile id ∈ `{ams, fgmc}`，决定 HLR 解析规则、分类关键词、追溯表配置与 AI 标注示例 |
...
| `controller_profile` 不在白名单 | 422 | `controller_profile: unsupported '<name>'; allowed: ams, fgmc` |
